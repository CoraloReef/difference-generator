...
const lineStatus: Record<string, (key: string, parent: string, value: unknown, valueOld?: unknown, valueNew?: unknown, children?: Record<string, unknown>[]) => string> = {
    ...
    changed: (key, parent, _value, valueOld, valueNew) => `Property '${repareParent(parent)}${key}' was updated. From ${repareValue(valueOld)} to ${repareValue(valueNew)}`,
    notChanged: () => '',
    parent: (key, parent, _value, _valueOld, _valueNew, children) => getLines(children || [], `${repareParent(parent)}${key}`).join('\n'),
};
